...
function drawBaseRenderLayer (ctx, layer) {
  ...
  if (layer.type === 'ellipse') {
    ctx.beginPath();
    ctx.ellipse(frame.x, frame.y, layer.rx, layer.ry, 0, 0, 2 * Math.PI);
    ctx.closePath();
  }
  ...
  if (layer.backgroundColor) {
    ...
    if (layer.borderRadius) {
      ...
      if (layer.type === 'ellipse') {
        ...
        ctx.ellipse(frame.x + halfWidth, frame.y + halfHeight, Math.abs(halfWidth), Math.abs(halfHeight), 0, 0, 2 * Math.PI);
        ctx.stroke();
        ctx.fill();
        ctx.closePath();
      } else {
